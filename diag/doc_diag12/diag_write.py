...
import os
import subprocess
from uploadiag12 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 12")
...
def retrieve_input():
    file = open('./diag/doc_diag12/diagrecap12.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag12/diagrecap12.txt'):
        importationFile('./diag/doc_diag12/diagrecap12.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap12.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag12/diag_read.py')
