...
class Application(Frame):
    ...
    def toDisplayError(self):
        ...
        self.MsgBox2msg = messagebox.showwarning("Warning",
            "entryfile8.txt or diagrecap8.txt doesn't exist !")
    ...
    def addLine(self):
        ...
        self.MSGBOX = messagebox.showinfo("Info",
            "Main Diagnosis from entryfile8.txt (DB) has been added, "\
            "click a second time on add !")

    def lastLap(self):
        """
            If diagrecap.txt created, main diagnosis from
            entryfile8.txt is added.
        """
        try:
            if os.path.getsize('./newpatient/entryfile8.txt'):
                print("[+] File 'entryfile8.txt' exist (add_2)!")
                with open('./newpatient/entryfile8.txt', 'r') as file_diag:
                ...
                with open("./diag/doc_diag8/diagrecap8.txt", "a+") as report_diag:
                    report_diag.write(line_e)
                print("[+] File diagrecap8.txt completed !")
                self.addLine()
        except FileNotFoundError as outdiag:
            print("[!] Sorry, file 'entryfile8.txt' doesn't exist !", outdiag)
            self.toDisplayError()
    ...
    def Frame_Ap1(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag8/diagrecap8.txt'):
                print("[+] File diagrecap8.txt exist (add)!")
                subprocess.run('./diag/doc_diag8/diag_write.py', check=True)
        except FileNotFoundError as outmsg:
            print("[!] Sorry, file diagrecap8.txt doesn't exist (add)!", outmsg)
            print("[+] File diag.txt created (add)!")
            with open('./diag/doc_diag8/diagrecap8.txt', 'w') as file:
                file.write("--- DIAGNOSTICS ---\n")
            print("[+] File diagrecap8.txt created !")
            self.lastLap()
    ...
    def Frame_Ap2(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag8/diagrecap8.txt'):
                print("[+] File diagrecap8.txt exist (read)!")
                subprocess.run('./diag/doc_diag8/diag_read.py', check=True)
        except FileNotFoundError as outcom:
            print("[!] Sorry, file diagrecap8.txt doesn't exist (read)!", outcom)
            self.toDisplayError()
