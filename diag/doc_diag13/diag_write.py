...
import os
import subprocess
from uploadiag13 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 13")
...
def retrieve_input():
    file = open('./diag/doc_diag13/diagrecap13.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag13/diagrecap13.txt'):
        importationFile('./diag/doc_diag13/diagrecap13.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap13.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag13/diag_read.py')
