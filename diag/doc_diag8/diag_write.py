...
import os
import subprocess
from uploadiag8 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 8")
...
def retrieve_input():
    file = open('./diag/doc_diag8/diagrecap8.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag8/diagrecap8.txt'):
        importationFile('./diag/doc_diag8/diagrecap8.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap8.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag8/diag_read.py')
