...
import os
import subprocess
from uploadiag16 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 16")
...
def retrieve_input():
    file = open('./diag/doc_diag16/diagrecap16.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag16/diagrecap16.txt'):
        importationFile('./diag/doc_diag16/diagrecap16.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap16.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag16/diag_read.py')
