...
import os
import subprocess
from uploadiag17 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 17")
...
def retrieve_input():
    file = open('./diag/doc_diag17/diagrecap17.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag17/diagrecap17.txt'):
        importationFile('./diag/doc_diag17/diagrecap17.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap17.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag17/diag_read.py')
