...
class Application(Frame):
    ...
    def toDisplayError(self):
        ...
        self.MsgBox2msg = messagebox.showwarning("Warning",
            "entryfile5.txt or diagrecap5.txt doesn't exist !")
    ...
    def addLine(self):
        ...
        self.MSGBOX = messagebox.showinfo("Info",
            "Main Diagnosis from entryfile5.txt (DB) has been added, "\
            "click a second time on add !")

    def lastLap(self):
        """
            If diagrecap.txt created, main diagnosis from
            entryfile5.txt is added.
        """
        try:
            if os.path.getsize('./newpatient/entryfile5.txt'):
                print("[+] File 'entryfile5.txt' exist (add_2)!")
                with open('./newpatient/entryfile5.txt', 'r') as file_diag:
                ...
                with open("./diag/doc_diag5/diagrecap5.txt", "a+") as report_diag:
                    report_diag.write(line_e)
                print("[+] File diagrecap5.txt completed !")
                self.addLine()
        except FileNotFoundError as outdiag:
            print("[!] Sorry, file 'entryfile5.txt' doesn't exist !", outdiag)
            self.toDisplayError()
    ...
    def Frame_Ap1(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag5/diagrecap5.txt'):
                print("[+] File diagrecap5.txt exist (add)!")
                subprocess.run('./diag/doc_diag5/diag_write.py', check=True)
        except FileNotFoundError as outmsg:
            print("[!] Sorry, file diagrecap5.txt doesn't exist (add)!", outmsg)
            print("[+] File diag.txt created (add)!")
            with open('./diag/doc_diag5/diagrecap5.txt', 'w') as file:
                file.write("--- DIAGNOSTICS ---\n")
            print("[+] File diagrecap5.txt created !")
            self.lastLap()
    ...
    def Frame_Ap2(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag5/diagrecap5.txt'):
                print("[+] File diagrecap5.txt exist (read)!")
                subprocess.run('./diag/doc_diag5/diag_read.py', check=True)
        except FileNotFoundError as outcom:
            print("[!] Sorry, file diagrecap5.txt doesn't exist (read)!", outcom)
            self.toDisplayError()
