...
import os
import subprocess
from uploadiag24 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 24")
...
def retrieve_input():
    file = open('./diag/doc_diag24/diagrecap24.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag24/diagrecap24.txt'):
        importationFile('./diag/doc_diag24/diagrecap24.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap24.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag24/diag_read.py')
