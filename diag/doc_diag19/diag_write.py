...
import os
import subprocess
from uploadiag19 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 19")
...
def retrieve_input():
    file = open('./diag/doc_diag19/diagrecap19.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag19/diagrecap19.txt'):
        importationFile('./diag/doc_diag19/diagrecap19.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap19.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag19/diag_read.py')
