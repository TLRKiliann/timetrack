...
import os
import subprocess
from uploadiag21 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 21")
...
def retrieve_input():
    file = open('./diag/doc_diag21/diagrecap21.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag21/diagrecap21.txt'):
        importationFile('./diag/doc_diag21/diagrecap21.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap21.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag21/diag_read.py')
