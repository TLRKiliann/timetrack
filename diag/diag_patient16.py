...
class Application(Frame):
    ...
    def toDisplayError(self):
        ...
        self.MsgBox2msg = messagebox.showwarning("Warning",
            "entryfile16.txt or diagrecap16.txt doesn't exist !")
    ...
    def addLine(self):
        ...
        self.MSGBOX = messagebox.showinfo("Info",
            "Main Diagnosis from entryfile16.txt (DB) has been added, "\
            "click a second time on add !")

    def lastLap(self):
        """
            If diagrecap.txt created, main diagnosis from
            entryfile16.txt is added.
        """
        try:
            if os.path.getsize('./newpatient/entryfile16.txt'):
                print("[+] File 'entryfile16.txt' exist (add_2)!")
                with open('./newpatient/entryfile16.txt', 'r') as file_diag:
                ...
                with open("./diag/doc_diag16/diagrecap16.txt", "a+") as report_diag:
                    report_diag.write(line_e)
                print("[+] File diagrecap16.txt completed !")
                self.addLine()
        except FileNotFoundError as outdiag:
            print("[!] Sorry, file 'entryfile16.txt' doesn't exist !", outdiag)
            self.toDisplayError()
    ...
    def Frame_Ap1(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag16/diagrecap16.txt'):
                print("[+] File diagrecap16.txt exist (add)!")
                subprocess.run('./diag/doc_diag16/diag_write.py', check=True)
        except FileNotFoundError as outmsg:
            print("[!] Sorry, file diagrecap16.txt doesn't exist (add)!", outmsg)
            print("[+] File diag.txt created (add)!")
            with open('./diag/doc_diag16/diagrecap16.txt', 'w') as file:
                file.write("--- DIAGNOSTICS ---\n")
            print("[+] File diagrecap16.txt created !")
            self.lastLap()
    ...
    def Frame_Ap2(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag16/diagrecap16.txt'):
                print("[+] File diagrecap16.txt exist (read)!")
                subprocess.run('./diag/doc_diag16/diag_read.py', check=True)
        except FileNotFoundError as outcom:
            print("[!] Sorry, file diagrecap16.txt doesn't exist (read)!", outcom)
            self.toDisplayError()
