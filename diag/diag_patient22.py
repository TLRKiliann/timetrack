...
class Application(Frame):
    ...
    def toDisplayError(self):
        ...
        self.MsgBox2msg = messagebox.showwarning("Warning",
            "entryfile22.txt or diagrecap22.txt doesn't exist !")
    ...
    def addLine(self):
        ...
        self.MSGBOX = messagebox.showinfo("Info",
            "Main Diagnosis from entryfile22.txt (DB) has been added, "\
            "click a second time on add !")

    def lastLap(self):
        """
            If diagrecap.txt created, main diagnosis from
            entryfile22.txt is added.
        """
        try:
            if os.path.getsize('./newpatient/entryfile22.txt'):
                print("[+] File 'entryfile22.txt' exist (add_2)!")
                with open('./newpatient/entryfile22.txt', 'r') as file_diag:
                ...
                with open("./diag/doc_diag22/diagrecap22.txt", "a+") as report_diag:
                    report_diag.write(line_e)
                print("[+] File diagrecap22.txt completed !")
                self.addLine()
        except FileNotFoundError as outdiag:
            print("[!] Sorry, file 'entryfile22.txt' doesn't exist !", outdiag)
            self.toDisplayError()
    ...
    def Frame_Ap2(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag22/diagrecap22.txt'):
                print("[+] File diagrecap22.txt exist (read)!")
                subprocess.run('./diag/doc_diag22/diag_read.py', check=True)
        except FileNotFoundError as outcom:
            print("[!] Sorry, file diagrecap22.txt doesn't exist (read)!", outcom)
            self.toDisplayError()
