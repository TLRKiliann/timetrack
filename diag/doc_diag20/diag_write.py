...
import os
import subprocess
from uploadiag20 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 20")
...
def retrieve_input():
    file = open('./diag/doc_diag20/diagrecap20.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag20/diagrecap20.txt'):
        importationFile('./diag/doc_diag20/diagrecap20.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap20.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag20/diag_read.py')
