...
import os
import subprocess
from uploadiag18 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 18")
...
def retrieve_input():
    file = open('./diag/doc_diag18/diagrecap18.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag18/diagrecap18.txt'):
        importationFile('./diag/doc_diag18/diagrecap18.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap18.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag18/diag_read.py')
