...
import os
import subprocess
from uploadiag10 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 10")
...
def retrieve_input():
    file = open('./diag/doc_diag10/diagrecap10.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag10/diagrecap10.txt'):
        importationFile('./diag/doc_diag10/diagrecap10.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap10.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag10/diag_read.py')
