...
class Application(Frame):
    ...
    def toDisplayError(self):
        ...
        self.MsgBox2msg = messagebox.showwarning("Warning",
            "entryfile4.txt or diagrecap4.txt doesn't exist !")
    ...
    def addLine(self):
        ...
        self.MSGBOX = messagebox.showinfo("Info",
            "Main Diagnosis from entryfile4.txt (DB) has been added, "\
            "click a second time on add !")

    def lastLap(self):
        """
            If diagrecap.txt created, main diagnosis from
            entryfile4.txt is added.
        """
        try:
            if os.path.getsize('./newpatient/entryfile4.txt'):
                print("[+] File 'entryfile4.txt' exist (add_2)!")
                with open('./newpatient/entryfile4.txt', 'r') as file_diag:
                ...
                with open("./diag/doc_diag4/diagrecap4.txt", "a+") as report_diag:
                    report_diag.write(line_e)
                print("[+] File diagrecap4.txt completed !")
                self.addLine()
        except FileNotFoundError as outdiag:
            print("[!] Sorry, file 'entryfile4.txt' doesn't exist !", outdiag)
            self.toDisplayError()
    ...
    def Frame_Ap2(self):
        ...
        try:
            if os.path.getsize('./diag/doc_diag4/diagrecap4.txt'):
                print("[+] File diagrecap4.txt exist (read)!")
                subprocess.run('./diag/doc_diag4/diag_read.py', check=True)
        except FileNotFoundError as outcom:
            print("[!] Sorry, file diagrecap4.txt doesn't exist (read)!", outcom)
            self.toDisplayError()
