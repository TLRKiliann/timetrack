...
import os
import subprocess
from uploadiag5 import diagupload


root = tk.Tk()
root.title("Diagnosis and ATCD 5")
...
def retrieve_input():
    file = open('./diag/doc_diag5/diagrecap5.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag5/diagrecap5.txt'):
        importationFile('./diag/doc_diag5/diagrecap5.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap5.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag5/diag_read.py')
