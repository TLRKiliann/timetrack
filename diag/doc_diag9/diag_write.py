...
root = tk.Tk()
root.title("Diagnosis and ATCD 9")
...
def retrieve_input():
    file = open('./diag/doc_diag9/diagrecap9.txt', 'w')
...
try:
    if os.path.getsize('./diag/doc_diag9/diagrecap9.txt'):
        importationFile('./diag/doc_diag9/diagrecap9.txt', 
            encodage="Utf-8")
except FileNotFoundError as err_file:
    print(err_file)
    messagebox.showwarning("WARNING", "File diagrecap9.txt does "\
        "not exist or file not found !")

def lectureFic():
    subprocess.call('./diag/doc_diag9/diag_read.py')
