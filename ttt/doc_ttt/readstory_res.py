...
def importationFile3(fichier3, encodage="Utf-8"):
    file3 = open(fichier3, 'r', encoding=encodage)
    content=file3.readlines()
    file3.close()
    for li3 in content:
        textBox.insert(END, li3)

fen=Tk()
fen.title("Historic of R")
...
labelo=Label(fen, text="Historic of R : ", width=15,
...
try:
    if os.path.getsize('./ttt/doc_ttt/intro_res.txt'):
        importationFile('./ttt/doc_ttt/intro_res.txt', encodage="Utf-8")
except FileNotFoundError as no_file4:
    print("+ File intro_res.txt not found !", no_file4)
    messagebox.showinfo('INFO', 'File intro_res.txt not found !')

try:
    if os.path.getsize('./ttt/doc_ttt/ires_rs.txt'):
        importationFile2('./ttt/doc_ttt/ires_rs.txt', encodage="Utf-8")
except FileNotFoundError as no_file4:
    print("+ File intro_res.txt not found !", no_file4)
...
try:
    if os.path.getsize('./ttt/doc_ttt/stopped_res.txt'):
        importationFile3('./ttt/doc_ttt/stopped_res.txt', encodage="Utf-8")
except FileNotFoundError as no_file4:
    print("+ File intro_res.txt not found !", no_file4)
    messagebox.showinfo('INFO', 'File stopped_res.txt not found !')
