...
def tttTabs():
    ...
    messagebox.showerror("Error", "No TREATMENT recorded for "\
        "this patient, convdose.json file missing (7)!")
...
def showTreat():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt7/convdose.json'):
            subprocess.run('./ttt/doc_ttt7/tabs.py')
...
def noReservStory():
    ...
    messagebox.showerror("Error", "No RESERVE recorded for "\
        "this patient, convres.json file missing (7)!")
...
def showReserve():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt7/convres.json'):
            subprocess.run('./ttt/doc_ttt7/tabres.py')
...
def introStoryTreat():
    ...
    messagebox.showerror("Error", "None stopped TREATMENT is "\
        "available, maybe no TREATMENT has been introduced (7)!")
...
def readTttStory():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt7/intro_ttt.txt"):
            subprocess.run("./ttt/doc_ttt7/readstory_ttt.py")
...
def introResStory():
    ...
    messagebox.showerror("Error", "None stopped RESERVE is "\
        "available, maybe no RESERVE has been introduced (7)!")
...
def readResStory():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt7/intro_res.txt"):
            subprocess.run("./ttt/doc_ttt7/readstory_res.py")
...
def readFileStory():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt7/stopped_ttt.txt"):
            subprocess.run("./ttt/doc_ttt7/readstory.py")
    except FileNotFoundError as no_storyfile:
        print("[!] Sorry, none stopped ttt is available, "\
        "maybe no ttt has been introduced (7)!", no_storyfile)
        introStoryTreat()
...
def noRecStop():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt7/ires_rs.txt"):
            subprocess.run("./ttt/doc_ttt7/stp_readstorymed.py")
    except FileNotFoundError as no_storyrestp:
        print("[!] Sorry, none RESERVE has been stopped (7)!", no_storyrestp)
        messagebox.showerror("Error", "None RESERVE has been stopped (7)!")
...
def showStopMed():
    ...
    try:
        ...
        print("[!] Sorry, none TREATMENT has been stopped (7)!", no_storyttstp)
        messagebox.showerror("Error", "None TREATMENT has been stopped (7)!")
        noRecStop()
