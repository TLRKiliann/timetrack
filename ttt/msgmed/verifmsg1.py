...
def tttTabs():
    ...
    messagebox.showerror("Error", "No TREATMENT recorded for "\
        "this patient, convdose.json file missing (1)!")
...
def showTreat():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt/convdose.json'):
            subprocess.run('./ttt/doc_ttt/tabs.py')
...
def noReservStory():
    ...
    messagebox.showerror("Error", "No RESERVE recorded for "\
        "this patient, convres.json file missing (1)!")
...
def showReserve():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt/convres.json'):
            subprocess.run('./ttt/doc_ttt/tabres.py')
...
def introStoryTreat():
    ...
    messagebox.showerror("Error", "None stopped TREATMENT is "\
        "available, maybe no TREATMENT has been introduced (1)!")
...
def readTttStory():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt/intro_ttt.txt"):
            subprocess.run("./ttt/doc_ttt/readstory_ttt.py")
...
def introResStory():
    ...
    messagebox.showerror("Error", "None stopped RESERVE is "\
        "available, maybe no RESERVE has been introduced (1)!")
...
def readResStory():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt/intro_res.txt"):
            subprocess.run("./ttt/doc_ttt/readstory_res.py")
...
def readFileStory():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt/stopped_ttt.txt"):
            subprocess.run("./ttt/doc_ttt/readstory.py")
    except FileNotFoundError as no_storyfile:
        print("[!] Sorry, none stopped ttt is available, "\
        "maybe no ttt has been introduced (1)!", no_storyfile)
        introStoryTreat()
...
def noRecStop():
    ...
    try:
        if os.path.getsize("./ttt/doc_ttt/ires_rs.txt"):
            subprocess.run("./ttt/doc_ttt/stp_readstorymed.py")
    except FileNotFoundError as no_storyrestp:
        print("[!] Sorry, none RESERVE has been stopped (1)!", no_storyrestp)
        messagebox.showerror("Error", "None RESERVE has been stopped (1)!")
...
def showStopMed():
    ...
    try:
        ...
        print("[!] Sorry, none TREATMENT has been stopped (1)!", no_storyttstp)
        messagebox.showerror("Error", "None TREATMENT has been stopped (1)!")
        noRecStop()
