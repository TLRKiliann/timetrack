...
def valFunc12(self):
    ...
    def stackvalidate(self):
        ...
        """
        try:
            with open('./validation/valfiles1/validate_12.txt', 'a+') as wcarefile:
                wcarefile.write('\n' + time.strftime('%d/%m/%Y'))
                wcarefile.write('\n' + self.patname.get() + ' - ')
                ...
                print("[+] Ok, data recorded into file validate_12.txt.")
        except FileNotFoundError as err_wc:
            print("[!] File validate_12.txt not found !", err_wc)
            with open('./validation/valfiles1/validate_12.txt', 'a+') as add_caref:
                add_caref.write('\n' + time.strftime('%d/%m/%Y'))
                add_caref.write('\n' + self.patname.get() + ' - ')
                ...
                print("[+] File validate_12.txt created !")
    ...
    try:
        ...
        print("[!] File entryfile12.txt doesn't exist !", callfile)
    ...
    def regroupAll():
        ...
        def importationTxtBox():
            """
                To import data from validate_12.txt
                and display in textbox2.
            """
            try:
                if os.path.getsize('./validation/valfiles1/validate_12.txt'):
                    print("[+] Ok, validate_12.txt exist")
            except FileNotFoundError as errfnf:
                print("[!] File validate_12.txt not found !", errfnf)
                with open('./validation/valfiles1/validate_12.txt', 'w'):
                    pass
                print("[+] File validate_12.txt created !")

            try:
                if os.path.exists('./validation/valfiles1/validate_12.txt'):
                    with open('./validation/valfiles1/validate_12.txt', 'r') as val_f:
                        nametxt = val_f.read()
            except (FileNotFoundError, UnboundLocalError) as err_nfvali:
                print("[!] File validate_12.txt not found !", err_nfvali)
    ...
    def validRightFrame(self):
        ...
        if self.firstcheck.get() == 1:
            print("ok validate")
            with open('./validation/valfiles1/validate_12.txt', 'a+') as oneval:
                oneval.write('\n' + self.datetov.get() + '\n')
