...
file = open('./param/aspifile24/temp.json')
data = json.load(file)
...
with open('./param/aspifile24/data_datetemp.json', 'a+') as datafile:
...
with open('./param/aspifile24/data_temp.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile24/temp.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt24/temp.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile24/plot/plot_temp.py', check=True)
