...
file = open('./param/aspifile17/temp.json')
data = json.load(file)
...
with open('./param/aspifile17/data_datetemp.json', 'a+') as datafile:
...
with open('./param/aspifile17/data_temp.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile17/temp.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt17/temp.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile17/plot/plot_temp.py', check=True)
