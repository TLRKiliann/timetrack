...
def writeData(textDate, textHour, textName, textTa, textDia,
    ...
    try:
        ...
        print("[!] File : 'paramdata7.txt' doesn't exist !", info)
...
def mainRead():
    try:
        if os.path.getsize('./param/main_read7.py'):
            subprocess.run('./param/main_read7.py', check=True)
    except FileNotFoundError as fnfe_read:
        print("[!] Sorry, file main_read7.py not found !")
        label['text'] = "Sorry, file main_read7.py not found !"
