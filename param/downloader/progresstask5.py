...
def process_of_unknown_duration(root):
    ...
    """
    #time.sleep(1)
    proc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt5/Files5/paramdata5.txt",
        "./param/"], stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("[+] File paramdata5.txt downloaded !")
        #tk.messagebox.showinfo("INFO", "paramdata5.txt downloaded")
    else:
        print("[!] No file to download !")
        tk.messagebox.showerror("Error", "No paramdata5.txt to download !")
