...
file = open('./param/aspifile13/systol.json')
data = json.load(file)
...
with open('./param/aspifile13/data_date.json', 'a+') as datafile:
...
with open('./param/aspifile13/data_Systol.json', 'a+') as datafile:
    json.dump(data_list2, datafile, indent=4)

#second file ttt
filedias = open('./param/aspifile13/diastol.json')
...
with open('./param/aspifile13/data_dia.json', 'a+') as datafiled:
...
with open('./param/aspifile13/data_Diastol.json', 'a+') as datafile2d:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile13/diastol.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt13/diastol.json"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(secproc.stderr))
    thirdproc = subprocess.run(["scp", "./param/aspifile13/systol.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt13/systol.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile13/plot/plot_prog.py', check=True)
