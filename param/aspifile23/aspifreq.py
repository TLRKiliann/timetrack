...
file = open('./param/aspifile23/freq.json')
data = json.load(file)
...
with open('./param/aspifile23/data_datefr.json', 'a+') as datafile:
...
with open('./param/aspifile23/data_fr.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile23/freq.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt23/freq.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile23/plot/plot_freq.py', check=True)
