...
import subprocess
import time
from downloader.progresstask9 import downloadata
from uploader.uploadbar import uploadmain
from uploader.upload9 import uploadata
...
def writeData(textDate, textHour, textName, textTa, textDia,
    ...
    try:
        ...
        print("[!] File : 'paramdata9.txt' doesn't exist !", info)
    ...
    try:
        if os.path.getsize('./param/aspifile9/systol.json'):
            ...
            with open('./param/aspifile9/systol.json', 'r') as datafile:
            ...
            if textTa.get() == "":
                ...
                with open('./param/aspifile9/systol.json', 'w') as datafile2:
    ...
    try:
        if os.path.getsize('./param/aspifile9/diastol.json'):
            ...
            if textDia.get() == "":
                ...
                with open('./param/aspifile9/diastol.json', 'w') as datafile2:
        ...
        if textDia.get() == "":
            ...
            print("[rec] 'Diastol' saved !")
            with open('./param/aspifile9/diastol.json', 'w') as datafile:
...
def mainRead():
    try:
        if os.path.getsize('./param/main_read9.py'):
            subprocess.run('./param/main_read9.py', check=True)
    except FileNotFoundError as fnfe_read:
        print("[!] Sorry, file main_read9.py not found !")
        label['text'] = "Sorry, file main_read9.py not found !"
...
def delSystolDia():
    ...
    try:
        if os.path.getsize('./param/aspifile9/systol.json'):
            with open('./param/aspifile9/systol.json', 'r') as filesys:
                data = json.load(filesys)
            for key, value in data.items():
                print("[del] Last systol deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile9/systol.json', 'w') as file:
    ...
    try:
        if os.path.getsize('./param/aspifile9/diastol.json'):
            with open('./param/aspifile9/diastol.json', 'r') as filedia:
                data = json.load(filedia)
            for key, value in data.items():
                print("[del] Last diastol deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile9/diastol.json', 'w') as file:
                data = json.dump(data, file, indent=4)
