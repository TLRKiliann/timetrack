...
file = open('./param/aspifile20/sat.json')
data = json.load(file)
...
with open('./param/aspifile20/data_datesat.json', 'a+') as datafile:
...
with open('./param/aspifile20/data_sat.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile20/sat.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt20/sat.json"],
        stderr=subprocess.PIPE)
...
print("\nLoading 'plot_sat.py'...")
subprocess.run('./param/aspifile20/plot/plot_sat.py', check=True)
