...
file = open('./param/aspifile20/temp.json')
data = json.load(file)
...
with open('./param/aspifile20/data_datetemp.json', 'a+') as datafile:
...
with open('./param/aspifile20/data_temp.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile20/temp.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt20/temp.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile20/plot/plot_temp.py', check=True)
