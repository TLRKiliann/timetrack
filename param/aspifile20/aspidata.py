...
file = open('./param/aspifile20/systol.json')
data = json.load(file)
...
with open('./param/aspifile20/data_date.json', 'a+') as datafile:
...
with open('./param/aspifile20/data_Systol.json', 'a+') as datafile:
    json.dump(data_list2, datafile, indent=4)

#second file ttt
filedias = open('./param/aspifile20/diastol.json')
...
with open('./param/aspifile20/data_dia.json', 'a+') as datafiled:
...
with open('./param/aspifile20/data_Diastol.json', 'a+') as datafile2d:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile20/diastol.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt20/diastol.json"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(secproc.stderr))
    thirdproc = subprocess.run(["scp", "./param/aspifile20/systol.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt20/systol.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile20/plot/plot_prog.py', check=True)
