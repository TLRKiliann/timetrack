...
file = open('./param/aspifile20/puls.json')
data = json.load(file)
...
with open('./param/aspifile20/data_datepuls.json', 'a+') as datafile:
...
with open('./param/aspifile20/data_puls.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile20/puls.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt20/puls.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile20/plot/plot_puls.py', check=True)
