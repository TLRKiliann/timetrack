...
def delMain():
    """
        To erase paramdata4.txt
    """
    Main_MsgBox = messagebox.askquestion("Confirm", "Are you sure ?\n"
        "It will delete paramdata4.txt with all data !!!")
    if Main_MsgBox == 'yes':
        try:
            if os.path.getsize('./param/paramdata4.txt'):
                os.remove('./param/paramdata4.txt')
                label['text'] = "File paramdata4.txt has been deleted !"
                print("[del] File paramdata4.txt has been deleted !")
