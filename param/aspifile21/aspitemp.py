...
file = open('./param/aspifile21/temp.json')
data = json.load(file)
...
with open('./param/aspifile21/data_datetemp.json', 'a+') as datafile:
...
with open('./param/aspifile21/data_temp.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile21/temp.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt21/temp.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile21/plot/plot_temp.py', check=True)
