...
file = open('./param/aspifile10/temp.json')
data = json.load(file)
...
with open('./param/aspifile10/data_datetemp.json', 'a+') as datafile:
...
with open('./param/aspifile10/data_temp.json', 'a+') as datafile:
    json.dump(data_list2, datafile, indent=4)

try:
    proc = subprocess.run(["scp", "./param/paramdata10.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt9/Files10/paramdata10.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    secproc = subprocess.run(["scp", "./param/aspifile10/temp.json",
...
subprocess.run('./param/aspifile10/plot/plot_temp.py', check=True)
