...
file = open('./param/aspifile14/puls.json')
data = json.load(file)
...
with open('./param/aspifile14/data_datepuls.json', 'a+') as datafile:
...
with open('./param/aspifile14/data_puls.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile14/puls.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt14/puls.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile14/plot/plot_puls.py', check=True)
