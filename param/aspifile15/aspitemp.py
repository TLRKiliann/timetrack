...
file = open('./param/aspifile15/temp.json')
data = json.load(file)
...
with open('./param/aspifile15/data_datetemp.json', 'a+') as datafile:
...
with open('./param/aspifile15/data_temp.json', 'a+') as datafile:
...
try:
    ...
    secproc = subprocess.run(["scp", "./param/aspifile15/temp.json",
        "pi@192.168.18.12:~/tt_doc/doc_txt15/temp.json"],
        stderr=subprocess.PIPE)
...
subprocess.run('./param/aspifile15/plot/plot_temp.py', check=True)
