...
import subprocess
import time
from downloader.progresstask3 import downloadata
from uploader.uploadbar import uploadmain
from uploader.upload3 import uploadata
import pygame
...
def writeData(textDate, textHour, textName, textTa, textDia,
    ...
    """
        ...
        and launching aspiFile3.py
    """
    play_rec()
    try:
        if os.path.getsize('./param/paramdata3.txt'):
            print("[+] File 'paramdata3.txt' exist !")
    except FileNotFoundError as info:
        print("[!] File : 'paramdata3.txt' doesn't exist !", info)
        print("[+] File 'paramdata3.txt' created !")
    finally:
        with open('./param/paramdata3.txt', 'a+') as file:
            file.write(str("Date : "))
    ...
    try:
        if os.path.getsize('./param/aspifile3/systol.json'):
            ...
            with open('./param/aspifile3/systol.json', 'r') as datafile:
            ...
            if textTa.get() == "":
                ...
                with open('./param/aspifile3/systol.json', 'w') as datafile2:
    ...
    try:
        if os.path.getsize('./param/aspifile3/diastol.json'):
            ...
            if textDia.get() == "":
                ...
                with open('./param/aspifile3/diastol.json', 'w') as datafile2:
        ...
        if textDia.get() == "":
            ...
            print("[rec] Ok 'Diastol' saved !")
            with open('./param/aspifile3/diastol.json', 'w') as datafile:
                json.dump(dataDia, datafile, indent=4)

    try:
        if os.path.getsize('./param/aspifile3/puls.json'):
            print("[+] File 'puls' exist !")
            with open('./param/aspifile3/puls.json', 'r') as datapuls:
            ...
            if textPuls.get() == "":
                ...
                with open('./param/aspifile3/puls.json', 'w') as datapuls2:
        ...
        if textPuls.get() == "":
            ...
            print("[rec] Ok 'Puls' saved !")
            with open('./param/aspifile3/puls.json', 'w') as datapuls3:
                json.dump(dataP, datapuls3, indent=4)

    try:
        if os.path.getsize('./param/aspifile3/sat.json'):
            print("[+] File 'sat' exist !")
            with open('./param/aspifile3/sat.json', 'r') as datasat:
            ...
            if textSa.get() == "":
                ...
                with open('./param/aspifile3/sat.json', 'w') as datasat2:
    ...
    try:
        if os.path.getsize('./param/aspifile3/freq.json'):
            ...
            if textFr.get() == "":
                ...
                with open('./param/aspifile3/freq.json', 'w') as datafreq2:
        ...
        if textFr.get() == "":
            ...
            print("[rec] Ok 'FR' saved !")
            with open('./param/aspifile3/freq.json', 'w') as datafreq3:
                json.dump(dataF, datafreq3, indent=4)

    try:
        if os.path.getsize('./param/aspifile3/temp.json'):
            print("[+] File 'temp' exist !")
            with open('./param/aspifile3/temp.json', 'r') as datatemp:
            ...
            if textTemp.get() == "":
                ...
                with open('./param/aspifile3/temp.json', 'w') as datatemp2:
    ...
    try:
        if os.path.getsize('./param/aspifile3/gly.json'):
            ...
            if textHgt.get() == "":
                ...
                with open('./param/aspifile3/gly.json', 'w') as datagly2:
    ...
    try:
        if os.path.getsize('./param/aspifile3/dlr.json'):
            ...
            if textDlrs.get() == "":
                ...
                with open('./param/aspifile3/dlr.json', 'w') as datadlr2:
        ...
        if textDlrs.get() == "":
            ...
            print("[rec] Ok 'Douleurs' saved !")
            with open('./param/aspifile3/dlr.json', 'w') as datadlr3:
...
def mainRead():
    subprocess.run('./param/main_read3.py', check=True)
...
def appelTens(textDate, textName, textTa, textDia):
    ...
    try:
        if os.path.getsize('./param/aspifile3/systol.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspidata.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
                "\nTension: " + textTa.get() + '/' + textDia.get())
...
def appelPuls(textDate, textName, textPuls):
    ...
    try:
        if os.path.getsize('./param/aspifile3/puls.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspipuls.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
...
def appelSat(textDate, textName, textSa):
    ...
    try:
        if os.path.getsize('./param/aspifile3/sat.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspisat.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
                "\nSaO2: " + textSa.get())
...
def appelFreq(textDate, textName, textFr):
    ...
    try:
        if os.path.getsize('./param/aspifile3/freq.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspifreq.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
...
def appelTemp(textDate, textName, textTemp):
    ...
    try:
        if os.path.getsize('./param/aspifile3/temp.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspitemp.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
                "\nTemperature: " + textTemp.get())
...
def appelGly(textDate, textName, textHgt):
    ...
    try:
        if os.path.getsize('./param/aspifile3/gly.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspigly.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
...
def appelDlr(textDate, textName, textDlrs):
    ...
    try:
        if os.path.getsize('./param/aspifile3/dlr.json'):
            uploadcall()
            subprocess.run('./param/aspifile3/aspidlr.py', check=True)
            label['text'] = ("Date: " + textDate.get() +
                "\nNom: " + textName.get() +
                "\nDouleurs: " + textDlrs.get())
...
def delMain():
    """
        To erase paramdata3.txt
    """
    Main_MsgBox = messagebox.askquestion("Confirm", "Are you sure ?\n"
        "It will delete paramdata3.txt with all data !!!")
    if Main_MsgBox == 'yes':
        try:
            if os.path.getsize('./param/paramdata3.txt'):
                os.remove('./param/paramdata3.txt')
                label['text'] = "File paramdata3.txt has been deleted !"
                print("[del] File paramdata3.txt has been deleted !")
...
def delSystolDia():
    ...
    try:
        if os.path.getsize('./param/aspifile3/systol.json'):
            with open('./param/aspifile3/systol.json', 'r') as filesys:
                data = json.load(filesys)
            for key, value in data.items():
                print("[del] Last systol deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/systol.json', 'w') as file:
    ...
    try:
        if os.path.getsize('./param/aspifile3/diastol.json'):
            with open('./param/aspifile3/diastol.json', 'r') as filedia:
                data = json.load(filedia)
            for key, value in data.items():
                print("[del] Last diastol deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/diastol.json', 'w') as file:
                json.dump(data, file, indent=4)
...
def delPuls():
    ...
    try:
        if os.path.getsize('./param/aspifile3/puls.json'):
            with open('./param/aspifile3/puls.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("[del] Last Puls deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/puls.json', 'w') as file:
                json.dump(data, file, indent=4)
...
def delSat():
    ...
    try:
        if os.path.getsize('./param/aspifile3/sat.json'):
            with open('./param/aspifile3/sat.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("[del] Last SaO2 deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/sat.json', 'w') as file:
                json.dump(data, file, indent=4)
...
def delFreq():
    ...
    try:
        if os.path.getsize('./param/aspifile3/freq.json'):
            with open('./param/aspifile3/freq.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("[del] Last FR deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/freq.json', 'w') as file:
                json.dump(data, file, indent=4)
...
def delTemp():
    ...
    try:
        if os.path.getsize('./param/aspifile3/temp.json'):
            with open('./param/aspifile3/temp.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("[del] Last Temp deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/temp.json', 'w') as file:
                json.dump(data, file, indent=4)
...
def delGly():
    ...
    try:
        if os.path.getsize('./param/aspifile3/gly.json'):
            with open('./param/aspifile3/gly.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("[del] Last Gly deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/gly.json', 'w') as file:
                json.dump(data, file, indent=4)
...
def delDlr():
    ...
    try:
        if os.path.getsize('./param/aspifile3/dlr.json'):
            with open('./param/aspifile3/dlr.json', 'r') as file:
                data = json.load(file)
            for key, value in data.items():
                print("[del] Last Pain deleted")
                print(value[-1])
                del value[-1]
            with open('./param/aspifile3/dlr.json', 'w') as file:
                json.dump(data, file, indent=4)
