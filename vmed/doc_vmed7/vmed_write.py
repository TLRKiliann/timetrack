...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed7/resultvmed7.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed7/resultvmed7.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt7/Files7/resultvmed7.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed7.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed7.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed7.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed7/resultvmed7.txt', 'r') as filereader:
...
try:
    if os.path.getsize('./vmed/doc_vmed7/resultvmed7.txt'):
        importationFile('./vmed/doc_vmed7/resultvmed7.txt',
            encodage="Utf-8")
