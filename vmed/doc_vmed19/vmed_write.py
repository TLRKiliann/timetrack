...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed19/resultvmed19.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed19/resultvmed19.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt19/Files19/resultvmed19.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed19.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed19.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed19.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed19/resultvmed19.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed19/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed19/resultvmed19.txt'):
        importationFile('./vmed/doc_vmed19/resultvmed19.txt',
            encodage="Utf-8")
