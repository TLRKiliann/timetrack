...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed24/resultvmed24.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed24/resultvmed24.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt24/Files24/resultvmed24.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed24.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed24.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed24.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed24/resultvmed24.txt', 'r') as filereader:
...
textBox=Text(root, height=15, width=80, font=18, relief=SUNKEN)
#textBox.insert(tk.INSERT, "\nEn date du : ")
#textBox.insert(tk.END, time.strftime("%d/%m/%Y à %H:%M:%S :\n"))
...
try:
    if os.path.getsize('./vmed/doc_vmed24/resultvmed24.txt'):
        importationFile('./vmed/doc_vmed24/resultvmed24.txt',
            encodage="Utf-8")
