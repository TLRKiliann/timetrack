...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed18/resultvmed18.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt18/Files18/resultvmed18.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed18.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed18.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed18.txt to upload...")
