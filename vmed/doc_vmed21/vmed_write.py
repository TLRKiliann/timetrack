...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed21/resultvmed21.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed21/resultvmed21.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt21/Files21/resultvmed21.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed21.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed21.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed21.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed21/resultvmed21.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed21/vmed_read.py')
