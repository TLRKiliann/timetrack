...
top = tk.Frame(root, bg='DodgerBlue3')
bottom = tk.Frame(root, bg='DodgerBlue3')
top.pack(side=tk.TOP)
bottom.pack(side=tk.BOTTOM, fill=BOTH, expand=YES)

labelo = tk.Label(root, text="Results of Medical Visit for : ",
    font='Arial 18 bold', fg='white', bg='DodgerBlue3')
...
labelallergy = tk.Label(root, text="Allergy",
    font='Arial 18 bold', fg='coral', bg='DodgerBlue3')
...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed16/resultvmed16.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed16/resultvmed16.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt16/Files16/resultvmed16.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed16.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed16.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed16.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed16/resultvmed16.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed16/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed16/resultvmed16.txt'):
        importationFile('./vmed/doc_vmed16/resultvmed16.txt',
            encodage="Utf-8")
