...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed15/resultvmed15.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt15/Files15/resultvmed15.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed15.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed15.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed15.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed15/resultvmed15.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed15/vmed_read.py')
