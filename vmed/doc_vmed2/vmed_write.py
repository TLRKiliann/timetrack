...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed2/resultvmed2.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed2/resultvmed2.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt2/Files2/resultvmed2.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed2.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed2.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed2.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed2/resultvmed2.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed2/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed2/resultvmed2.txt'):
        importationFile('./vmed/doc_vmed2/resultvmed2.txt',
            encodage="Utf-8")
