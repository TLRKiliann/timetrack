...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed6/resultvmed6.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed6/resultvmed6.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt6/Files6/resultvmed6.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed6.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed6.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed6.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed6/resultvmed6.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed6/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed6/resultvmed6.txt'):
        importationFile('./vmed/doc_vmed6/resultvmed6.txt',
            encodage="Utf-8")
