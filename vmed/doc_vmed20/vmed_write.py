...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed20/resultvmed20.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed20/resultvmed20.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt20/Files20/resultvmed20.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed20.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed20.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed20.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed20/resultvmed20.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed20/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed20/resultvmed20.txt'):
        importationFile('./vmed/doc_vmed20/resultvmed20.txt',
            encodage="Utf-8")
