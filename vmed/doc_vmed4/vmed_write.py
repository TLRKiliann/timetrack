...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed4/resultvmed4.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt4/Files4/resultvmed4.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed4.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed4.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed4.txt to upload...")
...
try:
    if os.path.getsize('./vmed/doc_vmed4/resultvmed4.txt'):
        importationFile('./vmed/doc_vmed4/resultvmed4.txt',
            encodage="Utf-8")
