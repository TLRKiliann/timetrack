...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed8/resultvmed8.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed8/resultvmed8.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt8/Files8/resultvmed8.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed8.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed8.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed8.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed8/resultvmed8.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed8/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed8/resultvmed8.txt'):
        importationFile('./vmed/doc_vmed8/resultvmed8.txt',
            encodage="Utf-8")
