...
def saveData():
    """
        ...
        it before.
    """
    with open('./vmed/doc_vmed13/resultvmed13.txt', 'a+') as filerecord:
...
def uploadfile():
    ...
    """
    proc = subprocess.run(["scp", "./vmed/doc_vmed13/resultvmed13.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt13/Files13/resultvmed13.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File resultvmed13.txt uploaded !")
        #messagebox.showinfo("INFO", "resultvmed13.txt uploaded...")
    else:
        print("+ No file to upload !")
        messagebox.showerror("Error", "No resultvmed13.txt to upload...")
...
def readerFile():
    ...
    """
    with open('./vmed/doc_vmed13/resultvmed13.txt', 'r') as filereader:
        print(filereader.read())
    subprocess.call('./vmed/doc_vmed13/vmed_read.py')
...
try:
    if os.path.getsize('./vmed/doc_vmed13/resultvmed13.txt'):
        importationFile('./vmed/doc_vmed13/resultvmed13.txt',
            encodage="Utf-8")
