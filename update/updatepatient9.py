...
def diagRecapt(diagnosis):
    try:
        if os.path.exists('./diag/doc_diag9/diagrecap9.txt'):
            tk.messagebox.showinfo("Info", "Data was updated for entryfile9.txt, "\
                "allergyfile9.txt !")
    except FileNotFoundError as not_ffile:
        print("[!] diagrecap9.txt not found !", not_ffile)
        with open('./diag/doc_diag9/diagrecap9.txt', 'w') as filediag:
            filediag.write(diagnosis + '\n')
        tk.messagebox.showwarning("WARNING", "File diagrecap9.txt created !")
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile9.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile9.txt to upload...")
