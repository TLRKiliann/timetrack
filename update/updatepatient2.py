...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient2.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.exists('./diag/doc_diag2/diagrecap2.txt'):
            tk.messagebox.showinfo("Info", "Data was updated for entryfile2.txt, "\
                "allergyfile2.txt !")
    except FileNotFoundError as not_ffile:
        print("[!] diagrecap2.txt not found !", not_ffile)
        with open('./diag/doc_diag2/diagrecap2.txt', 'w') as filediag:
            filediag.write(diagnosis + '\n')
        tk.messagebox.showwarning("WARNING", "File diagrecap2.txt created !")
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile2.txt uploaded...")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile2.txt to upload...")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    # Connection with database to update
    ...
    if idpatient == '2':
        if os.path.getsize('./newpatient/entryfile2.txt'):
            ...
            print("[+] File 'entryfile2.txt' is deleted !")
