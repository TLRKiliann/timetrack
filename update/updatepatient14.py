...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient14.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.exists('./diag/doc_diag14/diagrecap14.txt'):
            tk.messagebox.showinfo("Info", "Data was updated for entryfile14.txt, "\
                "allergyfile14.txt !")
    except FileNotFoundError as not_ffile:
        print("[!] diagrecap14.txt not found !", not_ffile)
        with open('./diag/doc_diag14/diagrecap14.txt', 'w') as filediag:
            filediag.write(diagnosis + '\n')
        tk.messagebox.showwarning("WARNING", "File diagrecap14.txt created !")
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile14.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile14.txt to upload...")
