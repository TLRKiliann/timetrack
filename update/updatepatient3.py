...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient3.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.exists('./diag/doc_diag3/diagrecap3.txt'):
            tk.messagebox.showinfo("Info", "Data was updated for entryfile3.txt, "\
                "allergyfile3.txt !")
    except FileNotFoundError as not_ffile:
        print("[!] diagrecap3.txt not found !", not_ffile)
        with open('./diag/doc_diag3/diagrecap3.txt', 'w') as filediag:
            filediag.write(diagnosis + '\n')
        tk.messagebox.showwarning("WARNING", "File diagrecap3.txt created !")
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile3.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile3.txt to upload...")
