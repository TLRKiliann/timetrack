...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient15.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.getsize('./diag/doc_diag15/diagrecap15.txt'):
            with open('./diag/doc_diag15/diagrecap15.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile15.txt, "\
                "allergyfile15.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap15.txt not found, plz create file clicking on diagnostic -")
        print(str(not_ffile))
        tk.messagebox.showwarning("WARNING", "File diagrecap15.txt not found ! "\
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile15.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile15.txt to upload...")
