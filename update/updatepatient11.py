...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient11.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.getsize('./diag/doc_diag11/diagrecap11.txt'):
            with open('./diag/doc_diag11/diagrecap11.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile11.txt, "\
                "allergyfile11.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap11.txt not found, plz create file clicking on diagnostic -")
        print(str(not_ffile))
        tk.messagebox.showwarning("WARNING", "File diagrecap11.txt not found ! "\
...
def searchLineName10(firstpat, surname, birthvalue,
    ...
    """
        ...
        entryfile11.txt and display
        tk.messagebox.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile11.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile11.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile11.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile11.txt to upload...")
