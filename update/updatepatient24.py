...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient24.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.exists('./diag/doc_diag24/diagrecap24.txt'):
            tk.messagebox.showinfo("Info", "Data was updated for entryfile24.txt, "\
                "allergyfile24.txt !")
    except FileNotFoundError as not_ffile:
        print("[!] diagrecap24.txt not found !", not_ffile)
        with open('./diag/doc_diag24/diagrecap24.txt', 'w') as filediag:
            filediag.write(diagnosis + '\n')
        tk.messagebox.showwarning("WARNING", "File diagrecap24.txt created !")
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile24.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile24.txt to upload...")
