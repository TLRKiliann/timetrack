...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient18.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.getsize('./diag/doc_diag18/diagrecap18.txt'):
            with open('./diag/doc_diag18/diagrecap18.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile18.txt, "\
                "allergyfile18.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap18.txt not found, plz create file clicking on diagnostic -")
        print(str(not_ffile))
        tk.messagebox.showwarning("WARNING", "File diagrecap18.txt not found ! "\
...
def searchLineName10(firstpat, surname, birthvalue,
    ...
    """
        ...
        entryfile18.txt and display
        tk.messagebox.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile18.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile18.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile18.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile18.txt to upload...")
