...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient12.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.getsize('./diag/doc_diag12/diagrecap12.txt'):
            with open('./diag/doc_diag12/diagrecap12.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile12.txt, "\
                "allergyfile12.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap12.txt not found, pls create file clicking on diagnostic -")
        print(str(not_ffile))
        tk.messagebox.showwarning("WARNING", "File diagrecap12.txt not found ! "\
...
def searchLineName10(firstpat, surname, birthvalue,
    ...
    """
        ...
        entryfile12.txt and display
        tk.messagebox.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile12.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile12.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile12.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile12.txt to upload...")
