...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient23.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.exists('./diag/doc_diag23/diagrecap23.txt'):
            tk.messagebox.showinfo("Info", "Data was updated for entryfile23.txt, "\
                "allergyfile23.txt !")
    except FileNotFoundError as not_ffile:
        print("[!] diagrecap23.txt not found !", not_ffile)
        with open('./diag/doc_diag23/diagrecap23.txt', 'w') as filediag:
            filediag.write(diagnosis + '\n')
        tk.messagebox.showwarning("WARNING", "File diagrecap23.txt created !")

def searchLineName10(firstpat, surname, birthvalue,
    allergia, transdisval, diagnosis):
    """
        To save changing data for
        entryfile23.txt and display
        tk.messagebox.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile23.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile23.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile23.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile23.txt to upload...")
