...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient16.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    """
        Modify main diagnosis in diagrecap_X.txt file.
    """
    try:
        if os.path.getsize('./diag/doc_diag16/diagrecap16.txt'):
            with open('./diag/doc_diag16/diagrecap16.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile16.txt, "\
                "allergyfile16.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap16.txt not found, plz create file clicking on diagnostic -")
        print(not_ffile)
        tk.messagebox.showwarning("WARNING", "File diagrecap16.txt not found ! "\
            "Please, create one by clicking on diagnostic 'add'.")

def searchLineName16(firstpat, surname, birthvalue,
    allergia, transdisval, diagnosis):
    """
        To save changing data for entryfile2.txt and display
        tk.messagebox.
        Call diagRecap() to modify diag file.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile16.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile16.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile16.txt uploaded...")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile16.txt to upload...")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '16':
        if os.path.getsize('./newpatient/entryfile16.txt'):
            ...
            print("[+] File 'entryfile16.txt' is deleted !")
            searchLineName16(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    funcrecord()
...
labelbirth = tk.Label(gui, text='Birth Date : ',
    font="Times 14 bold", fg='white', bg='DodgerBlue2')
