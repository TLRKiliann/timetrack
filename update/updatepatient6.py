...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient6.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.getsize('./diag/doc_diag6/diagrecap6.txt'):
            with open('./diag/doc_diag6/diagrecap6.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile6.txt, "\
                "allergyfile6.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap6.txt not found, plz create file clicking on diagnostic -")
        print(str(not_ffile))
        tk.messagebox.showwarning("WARNING", "File diagrecap6.txt not found ! "\
            "Please, create one by clicking on diagnostic 'add'.")

def searchLineName6(firstpat, surname, birthvalue,
    allergia, transdisval, diagnosis):
    """
        To save changing data for
        entryfile6.txt and display
        tk.messagebox.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile6.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile6.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile6.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile6.txt to upload...")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '6':
        if os.path.getsize('./newpatient/entryfile6.txt'):
            ...
            searchLineName6(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    funcrecord()
