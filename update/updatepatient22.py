...
try:
    ...
    print("[!] An error occured about pymysql - updatepatient22.py [!]", err_report)
    pass
...
def diagRecapt(diagnosis):
    try:
        if os.path.getsize('./diag/doc_diag22/diagrecap22.txt'):
            with open('./diag/doc_diag22/diagrecap22.txt', 'a+') as filediag:
                filediag.write(diagnosis + '\n')

            tk.messagebox.showinfo("Info", "Data was updated for entryfile22.txt, "\
                "allergyfile22.txt !")
    except FileNotFoundError as not_ffile:
        print("- diagrecap22.txt not found, plz create file clicking on diagnostic -")
        print(str(not_ffile))
        tk.messagebox.showwarning("WARNING", "File diagrecap22.txt not found ! "\
...
def searchLineName10(firstpat, surname, birthvalue,
    ...
    """
        ...
        entryfile22.txt and display
        tk.messagebox.
    """
    MsgBox = tk.messagebox.askyesno('Save data', 'Do you want to save ?')
    if MsgBox == 1:
        with open('./newpatient/entryfile22.txt', 'w') as fullfile:
        ...
        with open('./allergy/allergyfile22.txt', 'w') as filealler:
...
def funcrecord():
    ...
    if proc.stderr == b'':
        ...
        #tk.messagebox.showinfo("INFO", "entryfile22.txt uploaded...")
    else:
        print("+ No file to upload !")
        tk.messagebox.showerror("Error", "No entryfile22.txt to upload...")
