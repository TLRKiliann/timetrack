...
def recordaux(self):
    ...
    with open('./auxequip/doc_equip/auxiliary12.txt', 'a+') as fintro:
        fintro.write("----------------------------------------------------------\n")
        fintro.write("Date : ")
    ...
    with open('./auxequip/doc_equip/auxiliary12.txt', 'a+') as endfile:
        endfile.write("---------------------------------------------------------\n")
...
def uploadaux():
    ...
    """
    procaux = subprocess.run(["scp", "./auxequip/doc_equip/auxiliary12.txt",
        "pi@192.168.18.12:~/tt_doc/doc_txt12/Files12/auxiliary12.txt"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(procaux.stderr))
    if procaux.stderr == b'':
        print("[+] File auxiliary12.txt uploaded !")
        tk.messagebox.showinfo("INFO", "auxiliary12.txt uploaded...")
    else:
        print("[!] No file auxiliary12.txt to upload !")
        tk.messagebox.showerror("Error", "No auxiliary12.txt to upload...")
