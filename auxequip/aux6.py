...
def auxi_equip6(self):
    ...
    def recordaux():
        ...
        with open('./auxequip/doc_equip/auxiliary6.txt', 'a+') as file:
            file.write("----------------------------------------------------------\n")
            file.write("Date : ")
    ...
    def uploadaux():
        ...
        """
        proc = subprocess.run(["scp", "./auxequip/doc_equip/auxiliary6.txt",
            "pi@192.168.18.12:~/tt_doc/doc_txt6/Files6/auxiliary6.txt"],
            stderr=subprocess.PIPE)
        print("Result SCP transfert : %s" % repr(proc.stderr))
        if proc.stderr == b'':
            print("+ File auxiliary6.txt uploaded !")
            messagebox.showinfo("INFO", "auxiliary6.txt uploaded...")
        else:
            print("+ No file to upload !")
            messagebox.showerror("Error", "No auxiliary6.txt to upload...")
