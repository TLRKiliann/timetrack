...
def auxi_equip4(self):
    ...
    def recordaux():
        ...
        with open('./auxequip/doc_equip/auxiliary4.txt', 'a+') as file:
            file.write("----------------------------------------------------------\n")
            file.write("Date : ")
    ...
    def uploadaux():
        ...
        """
        proc = subprocess.run(["scp", "./auxequip/doc_equip/auxiliary4.txt",
            "pi@192.168.18.12:~/tt_doc/doc_txt4/Files4/auxiliary4.txt"],
            stderr=subprocess.PIPE)
        print("Result SCP transfert : %s" % repr(proc.stderr))
        if proc.stderr == b'':
            print("+ File auxiliary4.txt uploaded !")
            messagebox.showinfo("INFO", "auxiliary4.txt uploaded...")
        else:
            print("+ No file to upload !")
            messagebox.showerror("Error", "No auxiliary4.txt to upload...")
