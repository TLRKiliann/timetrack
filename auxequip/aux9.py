...
def auxi_equip9(self):
    ...
    def recordaux():
        ...
        with open('./auxequip/doc_equip/auxiliary9.txt', 'a+') as file:
            file.write("----------------------------------------------------------\n")
            file.write("Date : ")
    ...
    def uploadaux():
        ...
        """
        proc = subprocess.run(["scp", "./auxequip/doc_equip/auxiliary9.txt",
            "pi@192.168.18.12:~/tt_doc/doc_txt9/Files9/auxiliary9.txt"],
            stderr=subprocess.PIPE)
        print("Result SCP transfert : %s" % repr(proc.stderr))
        if proc.stderr == b'':
            print("+ File auxiliary9.txt uploaded !")
            messagebox.showinfo("INFO", "auxiliary9.txt uploaded...")
        else:
            print("+ No file to upload !")
            messagebox.showerror("Error", "No auxiliary9.txt to upload...")
