...
def recorderData(self, birthvar):
    ...
    """
    try:
        with open('./contact/conpact18/contact18.txt', 'w') as iofile:
    ...
    try:
        if os.path.getsize('./contact/conpact18/finalfile18.txt'):
            os.remove('./contact/conpact18/finalfile18.txt')
    except FileNotFoundError as err_termin:
        print("[!] finalfile18.txt not found !(Error_3)", err_termin)
        with open('./contact/conpact18/finalfile18.txt', 'a+'):
            print("[+] File finalfile18.txt created!")

    try:
        with open('./contact/conpact18/finalfile18.txt', 'w') as terminfile:
        ...
        print("[!] finalfile18.txt not created (Error_4)", err2_final)
