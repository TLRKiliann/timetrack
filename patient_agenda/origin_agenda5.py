...
if __name__=='__main__':
    ...
    class Control:
        def __init__(self, parent):
            ...
            with open('./newpatient/entryfile5.txt', 'r') as file_r:
        ...
        def print_selected_date(self):
            ...
            try:
                if os.path.getsize('./patient_agenda/events5/patient_calendar.txt'):
                    ...
                    file = open('./patient_agenda/events5/patient_calendar.txt','wb')
                    dump(self.data, file)
                    file.close()
                    subprocess.run('./patient_agenda/events5/entrer_event1.py', check=True)
        ...
        def accessDate(self):
            try:
                subprocess.run('./patient_agenda/events5/doc_events/fix_agenda/read_file.py', check=True)
        ...
        def accessLook(self):
            try:
                subprocess.run('./patient_agenda/events5/doc_events/fix_agenda/main.py', check=True)
