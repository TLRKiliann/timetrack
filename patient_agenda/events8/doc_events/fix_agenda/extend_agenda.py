...
def retrieve_input():
    ...
    print(inputValue)
    file_w = open('./patient_agenda/events8/doc_events/'\
    ...
    # 'agenda_saved' in 
    # './patient_agenda/events8/doc_events/fix_agenda' 
    topath = './patient_agenda/events8/doc_events/'\
    ...
    origin_path = './patient_agenda/events8/doc_events/'\
    'fix_agenda/fixed_rdv.txt'
    main_path = './patient_agenda/events8/doc_events/'\
    'fix_agenda/agenda_saved/'
    ...
    try:
        ...
        print("[Error] os.remove doesn't work for agenda 8", err_rm)
    ...
    try:
        ...
        print("[!] Loop for agenda 8 doesn't work !", err_loop)
    ...
    if secproc.stderr == b'':
        print("[Upload] './Backup/Files8' uploaded !")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "./Backup/Files8 not uploaded...")
...
def readerFile():
    ...
    """
    subprocess.run('./patient_agenda/events8/doc_events/'\
...
try:
    if os.path.getsize('./patient_agenda/events8/doc_events/'\
        'fix_agenda/patient_value.json'):
        print("[+] File 'patient_value.json exist' !")
        importationFile('./patient_agenda/events8/doc_events/'\
            'fix_agenda/patient_value.json')
