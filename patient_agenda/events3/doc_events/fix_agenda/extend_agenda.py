...
def retrieve_input():
    ...
    print(inputValue)
    file_w = open('./patient_agenda/events3/doc_events/'\
    ...
    try:
        ...
        print("[Error] os.remove doesn't work for agenda 3", err_rm)
    ...
    try:
        ...
        print("[!] Loop for agenda 3 doesn't work [!]", err_loop)
    ...
    except (OSError, FileNotFoundError) as e3:
        print("[!] No files from agenda_3 copied !!!", e3)

    secproc = subprocess.run(["scp", "-r", "-C",
        "./patient_agenda/events3/doc_events/fix_agenda/agenda_saved",
    ...
    if secproc.stderr == b'':
        ...
        tk.messagebox.showerror("Error", "./Backup/Files3 not uploaded...")
