...
def retrieve_input():
    ...
    print(inputValue)
    file_w = open('./patient_agenda/events/doc_events/'\
        'fix_agenda/fixed_rdv.txt', 'w')
    file_w.write(textBox.get("1.0", "end-1c") + '\n\n')
    file_w.close()

    # Create the directory 'agenda_saved' with path :
    ...
    for x in range(0, 1000):
        ...
        if not os.path.exists(main_path + allfiles[x]):
            ...
            print("[!] Out of range !!! (more than 1'000 files)")
            break
    ...
    try:
        ...
        print("[!] Loop for agenda_saved 1 doesn't work [!]", err_loop)
    ...
    except (OSError, FileNotFoundError) as err_backup:
        print("[!] No files from agenda_saved 1 copied !!!", err_backup)

    proc = subprocess.run(["scp", "-r", "-C",
        "./patient_agenda/events/doc_events/fix_agenda/agenda_saved",
        "pi@192.168.18.12:~/tt_doc/doc_txt1/Files1"], stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("[uploaded] './Backup/Files1' uploaded !")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "./Backup/Files1 not uploaded...")
