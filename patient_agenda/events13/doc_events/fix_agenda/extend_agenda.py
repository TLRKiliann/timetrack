...
def retrieve_input():
    ...
    print(inputValue)
    file_w = open('./patient_agenda/events13/doc_events/'\
    ...
    # 'agenda_saved' in 
    # './patient_agenda/events13/doc_events/fix_agenda' 
    topath = './patient_agenda/events13/doc_events/'\
    ...
    origin_path = './patient_agenda/events13/doc_events/'\
    'fix_agenda/fixed_rdv.txt'
    main_path = './patient_agenda/events13/doc_events/'\
    'fix_agenda/agenda_saved/'
    ...
    try:
        ...
        print("[Error] os.remove doesn't work for agenda 13", err_rm)

    print("[+] os.listdir after new file created : ")
    print(os.listdir('./patient_agenda/events13/doc_events/'\
        'fix_agenda/agenda_saved/'))
    ...
    try:
        ...
        print("[!] Loop for agenda 13 doesn't work !", err_loop)
    ...
    except (OSError, FileNotFoundError) as e13:
        print("[!] No files from agenda_13 copied !!!", e13)

    secproc = subprocess.run(["scp", "-r", "-C",
        "./patient_agenda/events13/doc_events/fix_agenda/agenda_saved",
        "pi@192.168.18.12:~/tt_doc/doc_txt13/Files13"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(secproc.stderr))
    if secproc.stderr == b'':
        print("[Upload] './Backup/Files13' uploaded !")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "./Backup/Files13 not uploaded...")
...
def readerFile():
    ...
    """
    subprocess.run('./patient_agenda/events13/doc_events/'\
...
try:
    if os.path.getsize('./patient_agenda/events13/doc_events/'\
        'fix_agenda/patient_value.json'):
        print("[+] File 'patient_value.json exist' !")
        importationFile('./patient_agenda/events13/doc_events/'\
            'fix_agenda/patient_value.json')
