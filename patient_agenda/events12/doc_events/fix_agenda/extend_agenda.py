...
def retrieve_input():
    ...
    print(inputValue)
    file_w = open('./patient_agenda/events12/doc_events/'\
    ...
    # 'agenda_saved' in 
    # './patient_agenda/events12/doc_events/fix_agenda' 
    topath = './patient_agenda/events12/doc_events/'\
    ...
    origin_path = './patient_agenda/events12/doc_events/'\
    'fix_agenda/fixed_rdv.txt'
    main_path = './patient_agenda/events12/doc_events/'\
    'fix_agenda/agenda_saved/'
    ...
    try:
        ...
        print("[Error] os.remove doesn't work for agenda 12", err_rm)

    print("[+] os.listdir after new file created : ")
    print(os.listdir('./patient_agenda/events12/doc_events/'\
        'fix_agenda/agenda_saved/'))
    ...
    try:
        ...
        print("[!] Loop for agenda 12 doesn't work !", err_loop)
    ...
    except (OSError, FileNotFoundError) as e12:
        print("[!] No files from agenda_12 copied !!!", e12)

    secproc = subprocess.run(["scp", "-r", "-C",
        "./patient_agenda/events12/doc_events/fix_agenda/agenda_saved",
    ...
    if secproc.stderr == b'':
        ...
        tk.messagebox.showerror("Error", "./Backup/Files12 not uploaded...")
...
def readerFile():
    ...
    """
    subprocess.run('./patient_agenda/events12/doc_events/'\
...
try:
    if os.path.getsize('./patient_agenda/events12/doc_events/'\
        'fix_agenda/patient_value.json'):
        print("[+] File 'patient_value.json exist' !")
        importationFile('./patient_agenda/events12/doc_events/'\
            'fix_agenda/patient_value.json')
