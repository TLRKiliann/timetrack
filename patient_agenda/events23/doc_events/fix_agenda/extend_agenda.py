...
def retrieve_input():
    ...
    # 'agenda_saved' in 
    # './patient_agenda/events23/doc_events/fix_agenda' 
    topath = './patient_agenda/events23/doc_events/'\
    ...
    origin_path = './patient_agenda/events23/doc_events/'\
    ...
    try:
        ...
        print("[Error] os.remove doesn't work for agenda 23", err_rm)
    ...
    if secproc.stderr == b'':
        print("[Upload] './Backup/Files23' uploaded !")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "./Backup/Files23 not uploaded...")
...
def readerFile():
    ...
    """
    subprocess.run('./patient_agenda/events23/doc_events/'\
...
try:
    if os.path.getsize('./patient_agenda/events23/doc_events/'\
        'fix_agenda/patient_value.json'):
        print("[+] File 'patient_value.json exist' !")
        importationFile('./patient_agenda/events23/doc_events/'\
            'fix_agenda/patient_value.json')
