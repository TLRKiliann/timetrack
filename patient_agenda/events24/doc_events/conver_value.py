...
try:
    with open('./patient_agenda/events24/doc_events/patient_rdv.json') as file:
        data=json.load(file)
except FileNotFoundError as fileout:
    print("File 24 patient_rdv.json not created", fileout)
...
try:
    if os.path.getsize('./patient_agenda/events24/doc_events/fix_agenda/patient_value.json'):
        ...
        with open('./patient_agenda/events24/doc_events/fix_agenda/patient_value.json','w') as partytime:
            json.dump(final_data, partytime)
except FileNotFoundError as msg:
    print("File doesn't exist, but it has been created !")
    with open('./patient_agenda/events24/doc_events/fix_agenda/patient_value.json','w') as partyleft:
    	json.dump(final_data, partyleft)

subprocess.call('./patient_agenda/events24/doc_events/fix_agenda/extend_agenda.py')
