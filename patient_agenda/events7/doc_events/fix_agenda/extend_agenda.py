...
def retrieve_input():
    ...
    print(inputValue)
    file_w = open('./patient_agenda/events7/doc_events/'\
    ...
    try:
        ...
        print("[Error] os.remove doesn't work for agenda 7", err_rm)
    ...
    try:
        ...
        print("[!] Loop for agenda 7 doesn't work !", err_loop)
    ...
    except (OSError, FileNotFoundError) as e7:
        print("[!] No files from agenda_7 copied !!!", e7)

    secproc = subprocess.run(["scp", "-r", "-C",
        "./patient_agenda/events7/doc_events/fix_agenda/agenda_saved",
        "pi@192.168.18.12:~/tt_doc/doc_txt7/Files7"],
        stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(secproc.stderr))
    if secproc.stderr == b'':
        print("[Upload] './Backup/Files7' uploaded !")
    else:
        print("[!] No file to upload !")
        tk.messagebox.showerror("Error", "./Backup/Files7 not uploaded...")
...
try:
    if os.path.getsize('./patient_agenda/events7/doc_events/'\
        ...
        importationFile('./patient_agenda/events7/doc_events/'\
            'fix_agenda/patient_value.json')
