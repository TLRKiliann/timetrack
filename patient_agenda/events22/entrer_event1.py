...
file=open('./patient_agenda/events22/patient_calendar.txt','rb')
data=load(file)
file.close()
...
try:
    if os.path.getsize('./patient_agenda/events22/doc_events/patient_rdv.json'):
        ...
        with open('./patient_agenda/events22/doc_events/patient_rdv.json', 'w') as datafile:
...
subprocess.call('./patient_agenda/events22/doc_events/conver_value.py')
