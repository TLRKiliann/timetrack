...
import os
import subprocess
from upload10 import needuploadata
...
def suiteBackup():
    ...
    """
    with open('./need/doc_suivi10/patient10_14b.txt', 'w') as namefile:
...
def saveData():
    ...
    try:        
        if os.path.getsize('./need/doc_suivi10/main_14b.txt'):
            ...
            with open('./need/doc_suivi10/main_14b.txt', 'a+') as namefile:
...
def lectureFic():
    """
        To read file.
    """
    with open('./need/doc_suivi10/patient10_14b.txt', 'r') as f1read:
        print(f1read.read())
    subprocess.run('./need/doc_suivi10/patient10_read.py', check=True)
...
def importationFile(fichier, encodage="Utf-8"):
    ...
    try:        
        if os.path.getsize(fichier):
            print("[+] File 'patient10_14b.txt' exist !")
