...
import os
import subprocess
from upload7 import needuploadata
...
def saveData():
    ...
    try:        
        if os.path.getsize('./need/doc_suivi7/main_14b.txt'):
            ...
            with open('./need/doc_suivi7/main_14b.txt', 'a+') as namefile:
...
def lectureFic():
    """
        To read file.
    """
    with open('./need/doc_suivi7/patient7_14b.txt', 'r') as f1read:
        print(f1read.read())
    subprocess.run('./need/doc_suivi7/patient7_read.py', check=True)
...
try:
    if os.path.getsize('./need/doc_suivi7/patient7_14b.txt'):
        importationFile('./need/doc_suivi7/patient7_14b.txt',
            encodage='Utf-8')
