...
import os
import subprocess
from upload8 import needuploadata
...
def suiteBackup():
    ...
    """
    with open('./need/doc_suivi8/patient8_14b.txt', 'w') as namefile:
...
def saveData():
    ...
    try:        
        if os.path.getsize('./need/doc_suivi8/main_14b.txt'):
            ...
            with open('./need/doc_suivi8/main_14b.txt', 'a+') as namefile:
...
def lectureFic():
    """
        To read file.
    """
    with open('./need/doc_suivi8/patient8_14b.txt', 'r') as f1read:
        print(f1read.read())
    subprocess.run('./need/doc_suivi8/patient8_read.py', check=True)
...
def importationFile(fichier, encodage="Utf-8"):
    ...
    try:        
        if os.path.getsize(fichier):
            print("[+] File 'patient8_14b.txt' exist !")
...
try:
    if os.path.getsize('./need/doc_suivi8/patient8_14b.txt'):
        importationFile('./need/doc_suivi8/patient8_14b.txt',
            encodage='Utf-8')
