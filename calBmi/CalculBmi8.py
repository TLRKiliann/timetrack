...
from bmi_download.progresstask8 import downloadata
from bmi_upload.uploadbar import uploadmain
from bmi_upload.upload8 import uploadata
...
def readBmi():
    try:
        if os.path.getsize('./calBmi/bmi_read8.py'):
            subprocess.run('./calBmi/bmi_read8.py', check=True)
    except FileNotFoundError as err_fnfread:
        print("[!] bmi_read8.py file doesn't exist !", err_fnfread)
        tk.messagebox.showinfo('INFO', 'File bmi_read8.py not found !')
