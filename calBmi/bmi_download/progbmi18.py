...
def procesSubBmi(root):
    ...
    """
    time.sleep(1)
    proc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt18/Files18/bmi18.txt",
        "./calBmi/"], stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("[Download] File bmi18.txt downloaded !")
        #tk.messagebox.showinfo("INFO", "bmi18.txt downloaded")
    else:
        print("[!] No file to download !")
        tk.messagebox.showerror("Error", "No bmi18.txt to download")

    secproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt18/Files18/file_kg.json",
        "./calBmi/doc_BMI18/"], stderr=subprocess.PIPE)
    ...
    thirdproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt18/Files18/file_bmi.json",
        "./calBmi/doc_BMI18/"], stderr=subprocess.PIPE)
...
def downloadBmi18():
    ...
    t1.start()
    print("[ Downloading BMI_18 start ]")
    task(root)
    t1.join()
    root.destroy()
