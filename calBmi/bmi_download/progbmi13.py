...
def procesSubBmi(root):
    ...
    """
    time.sleep(1)
    proc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt13/Files13/bmi13.txt",
        "./calBmi/"], stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("[Download] File bmi13.txt downloaded !")
        #tk.messagebox.showinfo("INFO", "bmi13.txt downloaded")
    else:
        print("[!] No file to download !")
        tk.messagebox.showerror("Error", "No bmi13.txt to download")

    secproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt13/Files13/file_kg.json",
        "./calBmi/doc_BMI13/"], stderr=subprocess.PIPE)
    ...
    thirdproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt13/Files13/file_bmi.json",
        "./calBmi/doc_BMI13/"], stderr=subprocess.PIPE)
...
def downloadBmi13():
    ...
    t1.start()
    print("[ Downloading BMI_13 start ]")
    task(root)
    t1.join()
    root.destroy()
