...
def procesSubBmi(root):
    ...
    if proc.stderr == b'':
        print("[Download] File bmi16.txt downloaded !")
        #tk.messagebox.showinfo("INFO", "bmi16.txt downloaded")
    else:
        print("[!] No file to download !")
        tk.messagebox.showerror("Error", "No bmi16.txt to download")
...
def downloadBmi16():
    ...
    t1.start()
    print("[ Downloading BMI_16 start ]")
    task(root)
    t1.join()
    root.destroy()
