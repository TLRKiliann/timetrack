...
def procesSubBmi(root):
    ...
    """
    time.sleep(1)
    proc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt19/Files19/bmi19.txt",
        "./calBmi/"], stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("[Download] File bmi19.txt downloaded !")
        #tk.messagebox.showinfo("INFO", "bmi19.txt downloaded")
    else:
        print("[!] No file to download !")
        tk.messagebox.showerror("Error", "No bmi19.txt to download")

    secproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt19/Files19/file_kg.json",
        "./calBmi/doc_BMI19/"], stderr=subprocess.PIPE)
    ...
    thirdproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt19/Files19/file_bmi.json",
        "./calBmi/doc_BMI19/"], stderr=subprocess.PIPE)
...
def downloadBmi19():
    ...
    t1.start()
    print("[ Downloading BMI_19 start ]")
    task(root)
    t1.join()
    root.destroy()
