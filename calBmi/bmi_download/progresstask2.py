...
def process_of_unknown_duration(root):
    ...
    """
    time.sleep(1)
    proc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt2/Files2/bmi2.txt",
        "./calBmi/"], stderr=subprocess.PIPE)
    print("Result SCP transfert : %s" % repr(proc.stderr))
    if proc.stderr == b'':
        print("+ File bmi2.txt downloaded !")
        #messagebox.showinfo("INFO", "bmi7.txt downloaded")
    else:
        print("+ No file to download !")
        messagebox.showerror("Error", "No bmi2.txt to download")

    secproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt2/Files2/file_kg.json",
        "./calBmi/doc_BMI2/"], stderr=subprocess.PIPE)
    ...
    thirdproc = subprocess.run(["scp", "pi@192.168.18.12:~/tt_doc/doc_txt2/Files2/file_bmi.json",
        "./calBmi/doc_BMI2/"], stderr=subprocess.PIPE)
