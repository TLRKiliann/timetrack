...
from bmi_download.progresstask17 import downloadata
from bmi_upload.uploadbar import uploadmain
from bmi_upload.upload17 import uploadata
...
def launchSaver():
    ...
    with open('./calBmi/bmi17.txt', 'a+') as file:
        file.write(str("Date : "))
    ...
    try:
        if os.path.getsize('./calBmi/doc_BMI17/file_bmi.json'):
            ...
            with open('./calBmi/doc_BMI17/file_bmi.json', 'r') as datafile:
                datastore = json.load(datafile)
                print(datastore)
            dataBmi = datastore
            dataBmi['data'].append({'Date' : textDate.get(), 'BMI' : bypass})
            with open('./calBmi/doc_BMI17/file_bmi.json', 'w') as datafile2:
        ...
        dataBmi['data'] = []
        dataBmi['data'].append({'Date' : textDate.get(), 'BMI' : bypass})
        with open('./calBmi/doc_BMI17/file_bmi.json', 'w') as datafile:
            json.dump(dataBmi, datafile, indent=4)

    try:
        if os.path.getsize('./calBmi/doc_BMI17/file_kg.json'):
            print("[+] File 'Kg' exist !")
            with open('./calBmi/doc_BMI17/file_kg.json', 'r') as datafile:
                datastore = json.load(datafile)
                print(datastore)
            dataBmi = datastore
            dataBmi['data'].append({'Date' : textDate.get(), 'Kg' : entryNum1.get()})
            with open('./calBmi/doc_BMI17/file_kg.json', 'w') as datafile2:
                json.dump(dataBmi, datafile2, indent=4)
    except FileNotFoundError as outcom:
...
def readBmi():
    try:
        if os.path.getsize('./calBmi/bmi_read17.py'):
            subprocess.run('./calBmi/bmi_read17.py', check=True)
    except FileNotFoundError as err_fnfread:
        print("[!] bmi_read17.py file doesn't exist !", err_fnfread)
        tk.messagebox.showinfo('INFO', 'File bmi_read17.py not found !')
...
def buttdel():
    ...
    if MSB_War == 1:
        try:
            if os.path.getsize('./calBmi/doc_BMI17/file_bmi.json'):
                with open('./calBmi/doc_BMI17/file_bmi.json', 'r') as file:
                    data = json.load(file)
                for key, value in data.items():
                    print("[del] Last value of bmi deleted")
                    print(value[-1])
                    del value[-1]
                with open('./calBmi/doc_BMI17/file_bmi.json', 'w') as file:
        ...
        try:
            if os.path.getsize('./calBmi/doc_BMI17/file_kg.json'):
                with open('./calBmi/doc_BMI17/file_kg.json', 'r') as file:
                    data = json.load(file)
                for key, value in data.items():
                    print("[del] Last value of weight deleted")
                    print(value[-1])
                    del value[-1]
                with open('./calBmi/doc_BMI17/file_kg.json', 'w') as file:
                    data = json.dump(data, file, indent=4)
