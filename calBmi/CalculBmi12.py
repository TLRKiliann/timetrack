...
from bmi_download.progbmi12 import downloadBmi12
...
def readBmi():
    try:
        if os.path.getsize('./calBmi/bmi_read12.py'):
            subprocess.run('./calBmi/bmi_read12.py', check=True)
    except FileNotFoundError as err_fnfread:
        print("[!] bmi_read12.py file doesn't exist !", err_fnfread)
        tk.messagebox.showinfo('INFO', 'File bmi_read12.py not found !')
