...
from bmi_download.progbmi3 import downloadBmi3
...
def readBmi():
    try:
        if os.path.getsize('./calBmi/bmi_read3.py'):
            subprocess.run('./calBmi/bmi_read3.py', check=True)
    except FileNotFoundError as err_fnfread:
        print("[!] bmi_read3.py file doesn't exist !", err_fnfread)
        tk.messagebox.showinfo('INFO', 'File bmi_read3.py not found !')
