...
file = open('./calBmi/doc_BMI8/file_kg.json')
data = json.load(file)
#file.close
...
try:
    if os.path.getsize('./calBmi/doc_BMI8/customBmi.py'):
        subprocess.run('./calBmi/doc_BMI8/customBmi.py', check=True)
except FileNotFoundError as callfile1:
    print("+ File customBmi.py doesn't exist !", callfile1)

# to read into file the dates entered.
try:
    with open('./calBmi/doc_BMI8/custom_kg.txt', 'r') as namefile:
