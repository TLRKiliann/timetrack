...
try:
    if os.path.getsize('./calBmi/doc_BMI22/customBmi.py'):
        subprocess.run('./calBmi/doc_BMI22/customBmi.py', check=True)
except FileNotFoundError as callfile1:
    print("[!] File customBmi.py doesn't exist !", callfile1)

# to read into file the dates entered.
try:
    with open('./calBmi/doc_BMI22/custom_kg.txt', 'r') as namefile:
