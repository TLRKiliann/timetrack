...
from bmi_download.progresstask9 import downloadata
from bmi_upload.uploadbar import uploadmain
from bmi_upload.upload9 import uploadata
...
def launchSaver():
    ...
    with open('./calBmi/bmi9.txt', 'a+') as file:
        file.write(str("Date : "))
    ...
    try:
        if os.path.getsize('./calBmi/doc_BMI9/file_bmi.json'):
            ...
            with open('./calBmi/doc_BMI9/file_bmi.json', 'r') as datafile:
                datastore = json.load(datafile)
                print(datastore)
            dataBmi = datastore
            dataBmi['data'].append({'Date' : textDate.get(), 'BMI' : bypass})
            with open('./calBmi/doc_BMI9/file_bmi.json', 'w') as datafile2:
        ...
        dataBmi['data'] = []
        dataBmi['data'].append({'Date' : textDate.get(), 'BMI' : bypass})
        with open('./calBmi/doc_BMI9/file_bmi.json', 'w') as datafile:
            json.dump(dataBmi, datafile, indent=4)

    try:
        if os.path.getsize('./calBmi/doc_BMI9/file_kg.json'):
            print("[+] File 'Kg' exist !")
            with open('./calBmi/doc_BMI9/file_kg.json', 'r') as datafile:
                datastore = json.load(datafile)
                print(datastore)
            dataBmi = datastore
            dataBmi['data'].append({'Date' : textDate.get(), 'Kg' : entryNum1.get()})
            with open('./calBmi/doc_BMI9/file_kg.json', 'w') as datafile2:
                json.dump(dataBmi, datafile2, indent=4)
    except FileNotFoundError as outcom:
        ...
        dataBmi['data'] = []
        dataBmi['data'].append({'Date' : textDate.get(), 'Kg' : entryNum1.get()})
        with open('./calBmi/doc_BMI9/file_kg.json', 'w') as datafile:
            json.dump(dataBmi, datafile, indent=4)
    uploadfunc()
...
def viewGraphicBmi():
    try:
        if os.path.getsize('./calBmi/doc_BMI9/file_bmi.json'):
            subprocess.run('./calBmi/doc_BMI9/convert_bmilist.py', check=True)
...
def viewGraphicKilo():
    try:
        if os.path.getsize('./calBmi/doc_BMI9/file_kg.json'):
            subprocess.run('./calBmi/doc_BMI9/convert_kg.py', check=True)
    except FileNotFoundError as no_file:
...
def readBmi():
    try:
        if os.path.getsize('./calBmi/bmi_read9.py'):
            subprocess.run('./calBmi/bmi_read9.py', check=True)
    except FileNotFoundError as err_fnfread:
        print("[!] bmi_read9.py file doesn't exist !", err_fnfread)
        tk.messagebox.showinfo('INFO', 'File bmi_read9.py not found !')
