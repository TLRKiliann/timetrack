...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient19.py [!]", err_report)
    pass
...
def searchLineName19(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        ...
        with open('./allergy/allergyfile19.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile19.txt and "\
        "allergyfile19.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '19':
        if os.path.getsize('./newpatient/entryfile19.txt'):
            ...
            os.remove('./allergy/aller_drop19.txt')
            searchLineName19(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    gui.destroy()

with open('./allergy/aller_drop19.txt', 'r') as patfile:
