...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient14.py [!]", err_report)
    pass
...
def searchLineName14(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        with open('./newpatient/entryfile14.txt', 'w') as file2:
            file2.write(firstpat + " " + surname + '\n')
            file2.write(birthvalue + '\n')
            file2.write(allergia + '\n')
            file2.write(transdisval + '\n')
            file2.write(diagnosis + '\n')
        with open('./allergy/allergyfile14.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile14.txt and "\
        "allergyfile14.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '14':
        if os.path.getsize('./newpatient/entryfile14.txt'):
            ...
            os.remove('./allergy/aller_drop14.txt')
...
with open('./allergy/aller_drop14.txt', 'r') as patfile:
