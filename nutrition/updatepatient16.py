...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient16.py [!]", err_report)
    pass
...
def searchLineName16(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        with open('./newpatient/entryfile16.txt', 'w') as file2:
            file2.write(firstpat + " " + surname + '\n')
            file2.write(birthvalue + '\n')
            file2.write(allergia + '\n')
            file2.write(transdisval + '\n')
            file2.write(diagnosis + '\n')
        with open('./allergy/allergyfile16.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile16.txt and "\
        "allergyfile16.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '16':
        if os.path.getsize('./newpatient/entryfile16.txt'):
            ...
            os.remove('./allergy/aller_drop16.txt')
...
with open('./allergy/aller_drop16.txt', 'r') as patfile:
