...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient20.py [!]", err_report)
    pass
...
def searchLineName20(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        with open('./newpatient/entryfile20.txt', 'w') as file2:
            file2.write(firstpat + " " + surname + '\n')
            file2.write(birthvalue + '\n')
            file2.write(allergia + '\n')
            file2.write(transdisval + '\n')
            file2.write(diagnosis + '\n')
        with open('./allergy/allergyfile20.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile20.txt and "\
        "allergyfile20.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '20':
        if os.path.getsize('./newpatient/entryfile20.txt'):
            ...
            os.remove('./allergy/aller_drop20.txt')
            #os.remove('./allergy/allergyfile20.txt')
            searchLineName20(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    gui.destroy()

with open('./allergy/aller_drop20.txt', 'r') as patfile:
