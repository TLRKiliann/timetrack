...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient21.py [!]", err_report)
    pass
...
def searchLineName21(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        with open('./newpatient/entryfile21.txt', 'w') as file2:
            file2.write(firstpat + " " + surname + '\n')
            file2.write(birthvalue + '\n')
            file2.write(allergia + '\n')
            file2.write(transdisval + '\n')
            file2.write(diagnosis + '\n')
        with open('./allergy/allergyfile21.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile21.txt and "\
        "allergyfile21.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '21':
        if os.path.getsize('./newpatient/entryfile21.txt'):
            ...
            os.remove('./allergy/aller_drop21.txt')
            #os.remove('./allergy/allergyfile21.txt')
            searchLineName21(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    gui.destroy()

with open('./allergy/aller_drop21.txt', 'r') as patfile:
