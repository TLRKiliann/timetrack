...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient.py [!]", err_report)
    pass
...
def searchLineName(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    tk.messagebox.showinfo("Info", "Data was updated for entryfile.txt and "\
        "allergyfile.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if patient_num.get() == "" or firstname_pat.get() == "" or sur_pat.get() == "":
        tk.messagebox.showerror("MySQL Error", "Enter Correct Details.")
    else:
        sqlCon = pymysql.connect(host='192.168.18.12', port=3306, user='koala33',
            password='Ko@l@tr3379', database='timetrackconn')
        cur = sqlCon.cursor()
    ...
    if idpatient == '1':
        if os.path.getsize('./newpatient/entryfile.txt'):
            ...
            searchLineName(firstpat, surname, birthvalue, allergia,
