...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient15.py [!]", err_report)
    pass
...
def searchLineName15(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        with open('./newpatient/entryfile15.txt', 'w') as file2:
            file2.write(firstpat + " " + surname + '\n')
            file2.write(birthvalue + '\n')
            file2.write(allergia + '\n')
            file2.write(transdisval + '\n')
            file2.write(diagnosis + '\n')
        with open('./allergy/allergyfile15.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile15.txt and "\
        "allergyfile15.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '15':
        if os.path.getsize('./newpatient/entryfile15.txt'):
            ...
            os.remove('./allergy/aller_drop15.txt')
            #os.remove('./allergy/allergyfile15.txt')
            searchLineName15(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    gui.destroy()

with open('./allergy/aller_drop15.txt', 'r') as patfile:
