...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient22.py [!]", err_report)
    pass
...
def searchLineName22(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        with open('./newpatient/entryfile22.txt', 'w') as file2:
            file2.write(firstpat + " " + surname + '\n')
            file2.write(birthvalue + '\n')
            file2.write(allergia + '\n')
            file2.write(transdisval + '\n')
            file2.write(diagnosis + '\n')
        with open('./allergy/allergyfile22.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile22.txt and "\
        "allergyfile22.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '22':
        if os.path.getsize('./newpatient/entryfile22.txt'):
            ...
            os.remove('./allergy/aller_drop22.txt')
            #os.remove('./allergy/allergyfile22.txt')
            searchLineName22(firstpat, surname, birthvalue,
                allergia, transdisval, diagnosis)
    else:
        pass
    gui.destroy()

with open('./allergy/aller_drop22.txt', 'r') as patfile:
