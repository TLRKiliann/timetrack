...
def recordOption():
    ...
    if CheckVar1.get()==1:
        ...
        with open('./allergy/aller_drop9.txt', 'a+') as file:
            file.write("Gluten, ")
    else:
    ...
    if CheckVar16.get()==1:
        ...
        print("[!] Nothing has changes (intolerances)")
    subprocess.run('./nutrition/updatepatient9.py', check=True)
...
def saveCheck():
    ...
    """
    MSB = messagebox.askyesno('Save Data', 'Do you want to save data ?')
    if MSB == 1:
        recordOption()
        confRec()
        print("[+] Ok, data saved !")
