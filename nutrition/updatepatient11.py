...
try:
    ...
    print("[!] An error occured about pymysql - updatpatient11.py [!]", err_report)
    pass
...
def searchLineName11(firstpat, surname, birthvalue, allergia, transdisval, diagnosis):
    ...
    if MsgBox2 == 1:
        ...
        with open('./allergy/allergyfile11.txt', 'w') as al_file:
            al_file.write(allergia + '\n')
    tk.messagebox.showinfo("Info", "Data was updated for entryfile11.txt and "\
        "allergyfile11.txt !")
...
def uptopat(idpatient, patient_num, firstpat, firstname_pat,
    ...
    if idpatient == '11':
        if os.path.getsize('./newpatient/entryfile11.txt'):
            ...
            os.remove('./allergy/aller_drop11.txt')
...
with open('./allergy/aller_drop11.txt', 'r') as patfile:
